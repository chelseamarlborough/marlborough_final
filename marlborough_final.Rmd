...
Stokes (2015) finds that voters do pay attention to climate policy and afterwards penalize the incumbent governments for facilities viewed as harmful to the communities. I successfullly replicated Stokes' results. In my replication, I tested the strength of Stokes' model using a Bayesian regression model. I found that -

### Introduction

The first piece of this paper was a replication of Stokes' results. Here code and data are publically available on the Harvard Dataverse. In order to replicate her results, I ran her orignial code using R code. I was easily able to run the code for both the tables and figures, but the code for formatting the tables in LaTex was not provided on Dataverse. Instead, I opted to analyze the raw output of the regressions from my replications. All code for the replication is available in my GitHub repository.

After the replication, in my extension of Stoke's work I test the strength of Stokes' model by running a Bayesian regression upon it. In order to perform this treatment effect, I created a new variable named "effect" that is deprived from "perc_lib". According to Stokes' codebook, "perc_lib" represents the votes cast for the Liberal Party divided by the number of voters who cast ballots in the precinct. In other words, "perc_lib" is equivalent to vote share. If the vote share is greater than 0, it is equal to 1 in "effect". If the vote share is less than 0, it is equal to 0 in "effect". I then replaced Stokes' linear regressions with Bayesian ones and replaced "perc_lib" with "effect". 

Stokes' results in her figures and tables revealed that -
...
divisions$poll_id_07 <-
  as.factor(divisions$poll_id_07) #several variables need to be factors to ensure the functions run properly
...
vcovCluster_demean <- function(model, cluster, first)
{
  ...
  dfc <-
    (M / (M - 1)) * ((N - 1) / (N - K - first)) #NOTE: need to change this 6186 if the clusters is different
  uj  <-
...
# Cut out divisions with population density greater than 400 people per km^2 in 2007. N=3169.
...
ebalw <-
  replicate(N, 1) #for every treated unit, give the weight of 1.
...
d2011_d <-
d2011[which(d2011$avg_pwr > 0), ] # Only keeps units with windspeed data greater than 0.

d2007_d <-
d2007[which(d2007$prop_3km != 1), ] # Removes units already treated in 2007 (within 3 km of turbine).

ivpanel <-
rbind(d2007_d, d2011_d) # IV panel does not include 2003 data. It only looks at changes between 2007 and 2011. However, 2003 data is used for robustness checks in SI.
...
ivd$chng_lib <-
ivd$perc_lib - iv07$perc_lib # new DV for instrumental variable cross section -- see Table 2.
...
exact = c(F, F, F, F),
replace = FALSE
) # Finding 1 controls for each unit and not allowing reuse. Matches do not have to be exact.

matchobj_tr <-
ivd[match$index.treated, ] # these are the treated units

matchobj_ct <-
ivd[match$index.control, ] # these are the control units

matchobj <-
rbind(matchobj_tr, matchobj_ct) # matched dataset, for instrument analysis
...
mod_10 <-
  lm(perc_lib ~ op + Y2003 + Y2007,
     data = yx_diffed,
     weights = divisions$weights_o) # add in weights
...
```{r table 1 replication, include= FALSE}

set.seed(14)

yx_data <- divisions[, c("perc_lib", "prop")] %>% 
  mutate(effect = ifelse(perc_lib >= 0, "1", "0")) %>% 
...
#mod_1 <-stan_glm(effect ~ prop + Y2003 + Y2007, data = yx_diffed, refresh = 0, family = "binary")
...
yx_diffed <- yx_diffed %>% 
  mutate(test = ifelse(perc_lib >= 0, "1", "0")) %>% 
  mutate(test = as.numeric(test))

mod_2 <- stan_glm(test ~ op + Y2003 + Y2007, data = yx_diffed)
...
mod_3 <-
  stan_glm(
    test ~ prop + Y2003 + Y2007 + p_uni_degree + log_pop_denc + unemploy_rate + log_median_inc + p_immigrant,
    data = yx_diffed
  )
...
mod_4 <-
  stan_glm(
    test ~ op + Y2003 + Y2007 + p_uni_degree + log_pop_denc + unemploy_rate + log_median_inc + p_immigrant,
    data = yx_diffed
  )
...
mod_5 <- stan_glm(test ~ prop + Y2003 + Y2007, data = yx_diffed)
...
mod_6 <-
  stan_glm(test ~ op + Y2003 + Y2007, data = yx_diffed) #change treatment variable
...
mod_9 <-
  stan_glm(test ~ prop + Y2003 + Y2007,
     data = yx_diffed,
     weights = divisions$weights_p) # add in weights
